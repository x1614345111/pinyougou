...
//保存sku方法
public void saveItemList(GoodsVo goodsVo){
	...
	private void setItemValus(GoodsVo goodsVo, TbItem item) {
		...
		item.setGoodsId(goodsVo.getGoods().getId());//商品SPU编号
		item.setSellerId(goodsVo.getGoods().getSellerId());//商家编号
		item.setCategoryid(goodsVo.getGoods().getCategory3Id());//商品分类编号（3级）
